...
parser_arg.add_argument('--folder_structures1', type=str, required=True)
parser_arg.add_argument('--folder_structures2', type=str, required=True)
parser_arg.add_argument('--proportion', type=float, required=True)
args = parser_arg.parse_args()
folder_experiment = args.folder_experiment
folder_structures1 = args.folder_structures1
folder_structures2 = args.folder_structures2
...
if not is_homogeneous:
    structures1 = [folder_structures1 + path for path in os.listdir(folder_structures1) if ".pdb" in path]
    indexes1 = [int(name.split("/")[-1].split(".")[0].split("_")[-1]) for name in structures1]
    sorted_structures1 = [struct for _, struct in sorted(zip(indexes1, structures1))]

    structures2 = [folder_structures2 + path for path in os.listdir(folder_structures2) if ".pdb" in path]
...
renderer_mising = Renderer(pixels_x, pixels_y, N_atoms=968 * 3,
                    dfU=image_settings["renderer"]["dfU"], dfV=image_settings["renderer"]["dfV"],
                    dfang=image_settings["renderer"]["dfang"],
                    spherical_aberration=image_settings["renderer"]["spherical_aberration"],
                    accelerating_voltage=image_settings["renderer"]["accelerating_voltage"],
...
if is_homogeneous:
    backbone = torch.tensor(utils.get_backbone(centering_structure) - center_vector, dtype=torch.float32, device=device)
