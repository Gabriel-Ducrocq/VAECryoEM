...
def train(yaml_setting_path, debug_mode):
    ...
    for epoch in range(N_epochs):
        ...
        tracking_metrics = {"rmsd":[], "kl_prior_latent":[], "l2_pen":[], "continuity_loss":[], "clashing_loss":[]}
        ...
        for batch_num, (indexes, batch_images, batch_poses, batch_poses_translation) in enumerate(data_loader):
            ...
            predicted_structures = model.utils.deform_structure_bis(gmm_repr.mus, translation_per_residue, quaternions_per_domain, segments, device)
        ...
        if not debug_mode:
            model.utils.monitor_training(segments, tracking_metrics, epoch, experiment_settings, vae, optimizer)
