...
def train(yaml_setting_path, debug_mode):
    ...
    :return:
    """
    vae, image_translator, ctf, grid, gmm_repr, optimizer, dataset, N_epochs, batch_size, experiment_settings, latent_type, device, scheduler = model.utils.parse_yaml(yaml_setting_path)
    ...
    for epoch in range(N_epochs):
        ...
        for batch_num, (indexes, batch_images, batch_poses, batch_poses_translation) in enumerate(data_loader):
            ...
            predicted_structures = model.utils.deform_structure(gmm_repr.mus[:100], translation_per_residue,
            ...
            start_loss = time()
            print("Batch pred images", batch_predicted_images.shape)
