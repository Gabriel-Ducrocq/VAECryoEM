...
def parse_yaml(path):
    ...
    if type(particles_star) is dict and columns in particles_star:
        dataset = ImageDataSet(apix, Npix, particles_star["particles"], particles_path, down_side_shape=Npix_downsize, rad_mask=experiment_settings.get("input_mask_rad"))
    else:
...
class SpatialGridTranslate(torch.nn.Module):
    ...
    def transform(self, images: torch.Tensor, trans: torch.Tensor):
        ...
        print(trans.shape)
        print(einops.rearrange(self.coords, "N C2 -> 1 1 N C2").shape)
        print(einops.rearrange(trans, "B T C2 -> B T 1 C2").shape)
...
def monitor_training(mask, tracking_metrics, epoch, experiment_settings, vae, optimizer, pred_im, true_im):
    """
    ...
    true_image_wandb = wandb.Image(true_im,
                                         caption="True images")
    wandb.log({"Images/true_image": true_image_wandb})
    wandb.log({"Images/predicted_image": predicted_image_wandb})
    torch.save(vae, experiment_settings["folder_path"] + "models_isolde/full_model" + str(epoch))
