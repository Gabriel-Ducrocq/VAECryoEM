...
class VAE(torch.nn.Module):
    def __init__(self, encoder, decoder, device, mask_start_values, chains_info, N_domains=6, N_residues=1006, tau_mask=0.05,
                 latent_dim = None):
        ...
        self.n_chains = len(chains_info)
        assert mask_start_values["type"] == "uniform", "Only uniform starting values supported for now"
        if mask_start_values["type"] == "uniform":
            self.segments = {}
            for i in range(len(chains_info)):
                n_res = chains_info[f"segment{i}"]["n_res"] 
                n_segments = chains_info[f"segment{i}"]["n_segments"]
                bound_0 = n_res/n_segments
                self.segments[f"chain{i}"]["mean"] = torch.nn.Parameter(data=torch.tensor(np.array([bound_0/2 + i*bound_0 for i in range(n_segments)]),
                                                         dtype=torch.float32, device=device)[None, :], requires_grad=True)

                self.segments[f"chain{i}"]["std"] = torch.nn.Parameter(data= torch.tensor(np.ones(n_segments)*bound_0, dtype=torch.float32, device=device)[None,:],
                                                    requires_grad=True)

                self.segments[f"chain{i}"]["proportions"] = torch.nn.Parameter(data=torch.tensor(np.ones(n_segments) * 0, dtype=torch.float32, device=device)[None, :],
                                                requires_grad=True)

                self.segments[f"chain{i}"]["n_res"] = n_res
    ...
    def sample_mask(self, N_batch):
        """
        Samples a mask
        :return: dict of tensor(1, N_residues, N_segments), values of the segmentation for each chain.
        """
        segmentation_per_chain = {}
        for i in range(self.n_chains):
                residues = torch.arange(0, self.segments[f"chain{i}"]["n_res"], 1, dtype=torch.float32, device=device)[:, None]
                log_num = -0.5*(residues - self.segments[f"chain{i}"]["mean"])**2/self.segments[f"chain{i}"]["std"]**2 + \
                torch.log(self.segments[f"chain{i}"]["proportions"])
                segmentation = torch.softmax(log_num / self.tau_mask, dim=-1)
                segmentation_per_chain[f"chain{i}"] = segmentation

        return segmentation_per_chain
